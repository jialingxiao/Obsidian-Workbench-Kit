...
> 状态：**v0.4.2**。可视化看板、**51 个组件（7 大分类）**、**12 套中式主题**、4 套开箱模板。
...
| 模板 | 内容 | 默认主题 |
|---|---|---|
| **极简起步** | 问候 + 快捷入口 + 最近笔记 + 待办 | 不指定 |
| **知识库** | 报头 + 概览 + 精选 / 标签 / 枢纽 / 孤岛，9 个组件 | 朱砂 |
| **日程计划** | 时钟 + 日记 + 月历 + 到期待办，以「今天」为中心 | 素笺 |
| **写作台** | 目标 + 产出趋势 + 进行中的稿子 | 秋毫 |

完整参考：**[docs/presets.md](docs/presets.md)**。模板是 `_wb/presets/*.json`，格式和看板布局文件一样，自己加一个就会出现在列表里。套用时会跑一遍布局算法并按实际内容重新定高，所以手写坐标不必精确。
