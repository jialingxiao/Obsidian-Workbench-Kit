...
块的垂直行为是**放哪就是哪**：拖到哪儿就停在哪儿，块之间允许留空白。被压住的块会让开 —— 上下各找最近的落点、取近的那边，所以把 A 拖到 B 身上是**两块交换**，而不是把 B 往下顶一截（只会往下推的话，向上拖看着自然、向下拖却把板子越拖越长）。反过来那种「自动向上压紧、永不留洞」看着整齐，代价是你根本没法把块放在中间 —— 最上面的往下拖会弹回去，下面的往上拖会一路冲顶，整页的紧凑是算法说了算不是你说了算。想要紧凑排版时点工具栏的「⇅ 整理」手动压一次（可撤销）。

松手时会做一次**落点吸附**：离邻居的上下边缘或左右边缘只差一点点（纵向 2 行、横向 1 列）就贴上去，差得多就说明是你故意留的间距，原样不动。阈值刻意定得小 —— 放大只会让「我明明想留空」变成「它又自己跑了」，那正是自动压紧被换掉的原因。虚框实时显示吸附后的位置，所见即所得。
